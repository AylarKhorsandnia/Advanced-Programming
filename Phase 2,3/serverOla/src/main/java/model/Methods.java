...
public class Methods {
    ...
    public static String allTweets(User user) throws FileNotFoundException {
        ...
        for (int i = user.getTweetNumbers(); i > 0; i--) {
            res += (gson.fromJson(new FileReader("./src/main/resources/Tweets/" + user.getUsername() + "Tweets/" + i + ".json"), Tweet.class)).content;
            res += "\n";
            res += (gson.fromJson(new FileReader("./src/main/resources/Tweets/" + user.getUsername() + "Tweets/" + i + ".json"), Tweet.class)).getLikers().size() + " Likes     ";
            res += (gson.fromJson(new FileReader("./src/main/resources/Tweets/" + user.getUsername() + "Tweets/" + i + ".json"), Tweet.class)).getRetweeters().size() + " ReKhweets     ";
            res += "\n";
            res += "\n";
            res += "\n";
        }
